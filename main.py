...
def webhook():
    ...
    # 1) /start → привітання з оновленою клавіатурою
    if text.strip() == "/start":
        greeting = (
    ...
    # 4) Обробка будь-якого контенту після кнопок
    state = user_states.get(chat_id)
    user  = msg.get("from", {})
    header = f"✉️ Нове від {user.get('first_name','')} (@{user.get('username','')})\n"

    # Пересилання в лог-чат
    if "text" in msg:
        content = msg["text"]
        requests.post(f"{API_URL}/sendMessage", json={
            "chat_id": LOG_CHAT_ID,
        ...
        # Якщо користувач надіслав щось непрогнозоване
        requests.post(f"{API_URL}/sendMessage", json={
            "chat_id": chat_id,
            "text": "Будь ласка, скористайтеся кнопками для початку."
        })
        return "OK", 200

    # 5) Відповідь користувачу в залежності від стану
    if state == "petition":
